...
passport.use('local.signup',new LocalStrategy({ //aqui a dentro colocamos lo que queremos recibir
    ...
    //.User
    mysqlConnection.query("INSERT INTO `GeneracionTickets`.`User` (`NameUser`,`NumberPhone`, `Email`, `Password`, `Nickname`, `BirthdayDate`, `Ruc`, `Direccion`) VALUES ('"+[nombre]+"','"+[numbePhone]+"','"+[email]+"', '"+[newUser.password]+"', '"+[nickname]+"', '"+[birthayDate]+"', '"+[ruc]+"', '"+[direccion]+"');",(err,rows,filds)=>{
